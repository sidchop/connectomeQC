...
# Note that fsl_meants includes all voxels, this function only includes non-zero-voxels

r = getOption("repos")
r["CRAN"] = "http://cran.us.r-project.org"
options(repos = r)
list.of.packages <- c("optparse", "RNifti", "pracma")
...
option_list = list(
  make_option(c("-i", "--img"),  default=NA, type='character',
              help="Path to the 4d BOLD timeseries from which the timeseries
                  will be extracted."),
  make_option(c("-r", "--roi"), default=NA, type='character',
  ...
  make_option(c("-l", "--labels"), default=NA, type='character',
...
# 4. Write the output.
###################################################################
write.table(t(mmat), paste0(opt$work,'/',opt$subj,"_ts.txt"), 
